...
class Problem:
    ...
    def add_ms(self, date, user, pt1, pt2, pt3, pt4, pt5):
        cursor = self.db.cursor()
        sql = "INSERT INTO message(savetime,username,ft_one,ft_two,ft_three,ft_four,ft_five) VALUES ('%s','%s',%d,%d,%d,%d,%d)" % (date, user, pt1, pt2, pt3, pt4, pt5)
        # print sql
        try:
            cursor.execute(sql)
            self.db.commit()
...
def read_txt(file_name):
    ...
    while not done:
        ...
        ft_four = int(dump[6])
        ft_five = int(dump[7])
        pro.add_ms(data_time, user_name, ft_one, ft_two, ft_three, ft_four, ft_five)
