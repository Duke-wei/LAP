...
def safecutlp(x,leng):
    if x-5<0:
        return 0
    else:
        return x-5

def safecutrp(x,leng):
    if x+5>leng:
        return leng
    else:
        return x+5

def saveFaces2(image_name):
    faces = detectFaces(image_name)
    if faces:
        #将人脸保存在save_dir目录下。
        #Image模块：Image.open获取图像句柄，crop剪切图像(剪切的区域就是detectFaces返回的坐标)，save保存。
        ...
        count = 0
        for (x1,y1,x2,y2) in faces:
            file_name = os.path.join(save_dir,str(count)+"2.jpg")
            img = Image.open(image_name)

            #img[safecutrp(y1,img.width):safecutrp(y2,img.width),safecutlp(x1,img.height):safecutlp(x2,img.height)].save(file_name)
            #img[max(y1-5,0):min(y2+5,img.width),max(x1-5,0):min(x2+5,img.height)].save(file_name)
            img.crop((max(x1-5,0),max(y1-5,0),x2+5,y2+5)).save(file_name)
            #img[max(y1-5,0):y2+5,max(x1-5,0):x2+5].save(file_name)
            count+=1
